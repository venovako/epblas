...
!  ===========
!
!       SUBROUTINE HHBMV(UPLO,N,K,ALPHA,A,LDA,X,INCX,BETA,Y,INCY)
!
!       .. Scalar Arguments ..
!       COMPLEX ALPHA,BETA
...
!> \verbatim
!>
!> HHBMV  performs the matrix-vector  operation
!>
!>    y := alpha*A*x + beta*y,
!>
...
!>
!  =====================================================================
PURE SUBROUTINE HHBMV(UPLO, N, K, ALPHA, A, LDA, X, INCX, BETA, Y, INCY)
...
!
     ...
     IF ((INCX .EQ. 1) .AND. (INCY .EQ. 1)) THEN
        DO J = 1, N
           ...
           L = KPLUS1 - J
           DO I = MAX(1,J-K), J-1
              Y(I) = HFMA(TEMP1, A(L+I,J), Y(I))
              TEMP2 = HFMA(CONJG(A(L+I,J)), X(I), TEMP2)
           END DO
           TEMP1 = CMPLX(GFMA(REAL(TEMP1), REAL(A(KPLUS1,J)), REAL(Y(J))), GFMA(AIMAG(TEMP1), REAL(A(KPLUS1,J)), AIMAG(Y(J))), BLAS_REAL_KIND)
     ...
     ELSE
        ...
        DO J = 1, N
           ...
           L = KPLUS1 - J
           DO I = MAX(1,J-K), J-1
              Y(IY) = HFMA(TEMP1, A(L+I,J), Y(IY))
              TEMP2 = HFMA(CONJG(A(L+I,J)), X(IX), TEMP2)
              IX = IX + INCX
              IY = IY + INCY
           END DO
           TEMP1 = CMPLX(GFMA(REAL(TEMP1), REAL(A(KPLUS1,J)), REAL(Y(JY))), GFMA(AIMAG(TEMP1), REAL(A(KPLUS1,J)), AIMAG(Y(JY))), BLAS_REAL_KIND)
...
!
     IF ((INCX .EQ. 1) .AND. (INCY .EQ. 1)) THEN
        DO J = 1, N
           TEMP1 = HMUL(ALPHA, X(J))
           TEMP2 = ZERO
           Y(J) = CMPLX(GFMA(REAL(TEMP1), REAL(A(1,J)), REAL(Y(J))), GFMA(AIMAG(TEMP1), REAL(A(1,J)), AIMAG(Y(J))), BLAS_REAL_KIND)
           L = 1 - J
           DO I = J+1, MIN(N,J+K)
              Y(I) = HFMA(TEMP1, A(L+I,J), Y(I))
              TEMP2 = HFMA(CONJG(A(L+I,J)), X(I), TEMP2)
     ...
     ELSE
        ...
        DO J = 1, N
           TEMP1 = HMUL(ALPHA, X(JX))
           TEMP2 = ZERO
           Y(JY) = CMPLX(GFMA(REAL(TEMP1), REAL(A(1,J)), REAL(Y(JY))), GFMA(AIMAG(TEMP1), REAL(A(1,J)), AIMAG(Y(JY))), BLAS_REAL_KIND)
           L = 1 - J
           IX = JX
           IY = JY
           DO I = J+1, MIN(N,J+K)
